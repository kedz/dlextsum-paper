...
\hal{the following comment was commented out but not answered and idk why???? [[i'm not sure what you mean here. are you concatenating the two summaries? if so, say so. is that a little weird tho?]]}.
%This collection
...
\hal{is this right? this could yield something longer than the budget size because what if the last step selects a really long sentence.
  i think you want the argmax over $\{j \in [n] - S : \sum \dots \leq c\}$ to make things brief maybe define $\textit{len}$ to be the current summary length? or $\textit{len}(S)$? also minor quibble but i like $S \cup \{s_i\}$ better than without the braces.}
