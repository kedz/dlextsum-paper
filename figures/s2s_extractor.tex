...
%    \draw[->] (h_r_\step.north) -> (h_\step.south);
  ...
  \node[circle,draw=black,line width=0.4mm,minimum size=0.5cm] (attn1) at (2, 2.6) {};    
  \node[circle,draw=black,line width=0.4mm,minimum size=0.5cm] (attn2) at (4, 2.6) {};    
  \node[circle,draw=black,line width=0.4mm,minimum size=0.5cm] (attn3) at (6, 2.6) {};    
  \path[draw=black,-,line width=0.4mm] (attn1.north) -- (attn1.south);
  \path[draw=black,-,line width=0.4mm] (attn1.west) -- (attn1.east);
  \path[draw=black,-,line width=0.4mm] (attn2.north) -- (attn2.south);
  \path[draw=black,-,line width=0.4mm] (attn2.west) -- (attn2.east);
  \path[draw=black,-,line width=0.4mm] (attn3.north) -- (attn3.south);
  \path[draw=black,-,line width=0.4mm] (attn3.west) -- (attn3.east);
  
  \node[mlp={2}{orange}] (h1) at (2, 3.75) {};
  \node[mlp={2}{orange}] (h2) at (4, 3.75) {};
  \node[mlp={2}{orange}] (h3) at (6, 3.75) {};
  \node[circle, draw=blue, fill=blue!20] (y1) at (2, 4.8) {\tiny $y_1$};    
  \node[circle, draw=blue, fill=blue!20] (y2) at (4, 4.8) {\tiny $y_2$};    
  \node[circle, draw=blue, fill=blue!20] (y3) at (6, 4.8) {\tiny $y_3$};    


  \foreach \step in {1,...,3} {
      \node (m\step) at (\step * .9, 1.9) {};
      \path[draw=black!75,-] (h_f_\step.north) -- (m\step.center);
      \path[draw=black!75,-] (h_r_\step.north) -- (m\step.center);
      \foreach \stepj in {1,...,3} {
          \path[draw=black!75,->] (m\step.center) -- (attn\stepj.south);
      } 
  }
