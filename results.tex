...
flexibility to identify important word features. \hal{why is it surprising?} \textcolor{green}{[[CK: Because learning embeddings should give the model more capacity to represent important patterns]]}
...
%\hal{i'd label the rows as ``all words'' and then ``- nouns'', ``- verbs'', etc. also use paragraph not textbf :P.}
...
In examining the outputs of the models, we found
...
the Reddit domain, and shuffling actually improves performance on the AMI}.\hal{what about in the cross-domain setting?} This suggest that position 
